...
def task_daemon_able(task, data_center):
    ...
    if manager.GetUnitFileState(service) == 'generated' \
        or manager.GetUnitFileState(service) == 'masked':
        if operation == 'enable':
            manager.UnmaskUnitFiles([service], False)
        else:
            manager.MaskUnitFiles([service], False, False)
    else:
        if operation == 'enable':
